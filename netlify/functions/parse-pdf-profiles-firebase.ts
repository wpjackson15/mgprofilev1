...
export const handler: Handler = async (event) => {
  ...
  try {
    console.log('=== PDF Parsing Request (Firebase) ===');
    ...
    if (!pdfUrl) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'No PDF URL provided' }),
      };
    }
    ...
    if (!pdfResponse.ok) {
      console.error('Failed to download PDF from Firebase:', pdfResponse.status);
      throw new Error(`Failed to download PDF: ${pdfResponse.status}`);
    }

    const pdfBuffer = await pdfResponse.arrayBuffer();
    const pdfBase64 = Buffer.from(pdfBuffer).toString('base64');
    console.log('PDF downloaded and converted to base64, length:', pdfBase64.length);
